...
async function login(req, res) {
    try {
        ...
        logger.error(`Error during login for identifier ${req.body.identifier || 'unknown'}: ${error.message}`);
...
async function createUser(req, res) {
  try {
    ...
    if (!email?.trim() || !password?.trim() || !name?.trim() || !mobileNumber?.trim()) {
        logger.warn('Missing required fields in createUser request');
            return res.status(400).json({ message: 'Email, password, name, and mobile number are required.' });
    }
 
    if (email.trim()) {
      const emailExists = await User.findOne({ email: req.body.email.trim() });
      if (emailExists) {
        logger.info(`Attempted to create user with existing email: ${email.trim()}`);
        return res.status(400).json({ message: 'Email already exists.' });
      }
    }

    if (mobileNumber.trim()) {
      const mobileNumberExists = await User.findOne({ mobileNumber: req.body.mobileNumber.trim() });
    ...
    logger.error(`Error in createUser function: ${err.message}`, { stack: err.stack });
